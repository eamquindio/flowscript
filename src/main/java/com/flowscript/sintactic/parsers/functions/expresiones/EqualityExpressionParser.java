...
public class EqualityExpressionParser implements IParser<EqualityExpressionNode> {
    ...
    public EqualityExpressionNode parse(ParserContext context) throws Parser.ParseException {
        RelationalExpressionParser relationalExpressionParser = new RelationalExpressionParser();
        ExpressionNode left = relationalExpressionParser.parse(context);
        EqualityExpressionNode equalityNode = new EqualityExpressionNode(context.getCurrentToken(), left);
        while (context.checkAny(TokenType.EQUAL, TokenType.NOT_EQUAL)) {
            Token operator = context.consume();
            ExpressionNode right = relationalExpressionParser.parse(context);
            equalityNode.addOperand(operator, right);
        }
        return equalityNode;
    }
}
